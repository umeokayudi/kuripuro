...
export default function EmployeePortal() {
  ...
  const loadJobs = async () => {
    ...
    setJobs(data || [])
    const inProgress = data?.find(j => j.status === 'in_progress')
    if (inProgress) {
  ...
  const loadSalary = async () => {
    ...
    const totalValue = data.reduce((s, j) => s + Number(j.value || 0), 0)
    ...
    let estimated = 0
    if (user.salary_type === 'hourly') estimated = (totalMins / 60) * (user.hourly_rate || 0)
    else if (user.salary_type === 'fixed') estimated = user.fixed_salary || 0
    else estimated = (user.fixed_salary || 0) + totalValue * ((user.job_bonus_rate || 0) / 100)
    setSalaryData({ jobs: totalJobs, hours, value: totalValue, estimated: Math.round(estimated) })
  }
  ...
  const checkGPS = async (job) => {
    ...
    try {
      ...
      if (dist > 100) {
        setGpsStatus(`Too far: ${Math.round(dist)}m away (max 100m)`)
        toast.error(`You are ${Math.round(dist)}m from the job location`)
        return false
      }
      setGpsStatus(`✓ ${Math.round(dist)}m from location`)
      return true
    } catch (e) {
  ...
  const handleComplete = async () => {
    if (activeJob.photo_required && !photos.end) return toast.error('Photo required to complete this job')
    setSubmitting(true)
  ...
  const PhotoSlot = ({ slot, label, required }) => (
    <div>
      <input type="file" accept="image/*" capture="environment" ref={slot==='start'?startRef:endRef} style={{display:'none'}}
        onChange={e=>{const f=e.target.files[0];if(f){setPhotos(p=>({...p,[slot]:f}));setPreviews(p=>{URL.revokeObjectURL(p[slot]);return{...p,[slot]:URL.createObjectURL(f)}})}}} />
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0d2137',display:'flex',flexDirection:'column'}}>
      ...
      <div style={{display:'flex',borderBottom:'1px solid rgba(255,255,255,0.08)'}}>
        {[['jobs','📋 Jobs'],['salary','💴 Salary'],['history','📅 History']].map(([k,l])=>(
      ...
      <div style={{flex:1,padding:18,maxWidth:480,margin:'0 auto',width:'100%',overflowY:'auto'}}>
        ...
        {tab==='jobs' && <>
          ...
          {activeJob && (
            <div style={{marginBottom:16}}>
              <div style={{background:'rgba(193,156,86,0.15)',border:'1px solid rgba(193,156,86,0.4)',borderRadius:12,padding:'14px 16px',marginBottom:14}}>
                <div style={{fontSize:11,color:'rgba(255,255,255,0.5)',marginBottom:4}}>IN PROGRESS</div>
          ...
          {!activeJob && jobs.length === 0 && (
            <div style={{textAlign:'center',paddingTop:50,color:'rgba(255,255,255,0.4)'}}>
              ...
              <div>No jobs assigned for today</div>
            </div>
          )}
          ...
          {jobs.filter(j=>j.status==='assigned').map(j=>(
            <div key={j.id} style={{background:'rgba(255,255,255,0.05)',borderRadius:12,padding:'14px 16px',marginBottom:12}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'start',marginBottom:8}}>
                <div>
                  <div style={{fontSize:15,fontWeight:700,color:'#fff'}}>{j.title}</div>
                ...
                <div style={{textAlign:'right'}}>
                  <div style={{fontSize:14,fontWeight:700,color:'#c19c56'}}>¥{Number(j.value).toLocaleString()}</div>
                  {j.photo_required&&<div style={{fontSize:10,color:'#f87171',marginTop:4}}>📷 Photo req.</div>}
                </div>
              </div>
              ...
              <button onClick={()=>handleStart(j)} disabled={submitting||!!activeJob} style={{width:'100%',padding:'12px',borderRadius:10,border:'none',background: activeJob?'rgba(255,255,255,0.1)':'#0F6E56',color:'#fff',fontSize:14,fontWeight:700,cursor:activeJob?'not-allowed':'pointer',opacity:activeJob?.id!==j.id?0.6:1}}>
        ...
        {tab==='salary' && (
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            <div style={{fontSize:17,fontWeight:700,color:'#fff'}}>My Salary — {new Date().toLocaleString('en',{month:'long',year:'numeric'})}</div>
            {salaryData ? <>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                {[['Jobs done',salaryData.jobs],['Hours',salaryData.hours+'h'],['Job value','¥'+salaryData.value.toLocaleString()],['Estimated pay','¥'+salaryData.estimated.toLocaleString()]].map(([l,v])=>(
              ...
              <div style={{fontSize:11,color:'rgba(255,255,255,0.3)'}}>* Final amount confirmed by admin</div>
            ...
            {history.map(j=>(
              <div key={j.id} style={{background:'rgba(255,255,255,0.05)',borderRadius:10,padding:'12px 14px',marginBottom:10}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                  ...
                  <div style={{fontSize:13,fontWeight:600,color:'#c19c56'}}>¥{Number(j.value).toLocaleString()}</div>
                </div>
