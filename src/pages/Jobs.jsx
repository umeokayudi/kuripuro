...
export default function Jobs() {
  ...
  const [form, setForm] = useState({
    ...
    value:0, description:'', checklist_template:'', photo_required:false, location_id:''
  })
  ...
  const loadAll = async () => {
    ...
    const [j, e, c, l] = await Promise.all([
      ...
      supabase.from('employees').select('id,full_name,salary_type,hourly_rate').eq('is_active', true),
  ...
  const handleCreate = async () => {
    ...
    setForm({ title:'', client_id:'', client_name:'', employee_id:'', employee_name:'', scheduled_date:'', scheduled_time:'', address:'', gps_lat:'', gps_lng:'', value:0, description:'', checklist_template:'', photo_required:false, location_id:'' })
    loadAll()
    setTab('list')
  }
  ...
  return (
    <div>
      <div className="tab-pills">
        <button className={`tab-pill${tab==='list'?' active':''}`} onClick={()=>setTab('list')}>Jobs</button>
      ...
      {tab==='list' && (
        <div>
          ...
          {jobs.length === 0 && !loading && <div className="card"><div style={{color:'var(--text3)',fontSize:13}}>No jobs yet. Create one.</div></div>}
          {jobs.map(j => (
            <div key={j.id} className="card">
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'start',marginBottom:10}}>
                ...
                <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:6}}>
                  ...
                  <span style={{fontSize:13,fontWeight:600,color:'var(--green)'}}>¥{Number(j.value).toLocaleString()}</span>
                </div>
              </div>
              ...
              <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                ...
                {j.gps_lat && <span className="badge badge-navy">📍 GPS set</span>}
      ...
      {tab==='new' && (
        <div className="card">
          ...
          <div className="grid-2">
            ...
            <div className="form-group"><label>Job Value (¥)</label><input type="number" value={form.value} onChange={e=>upd('value',e.target.value)} /></div>
          ...
          <div className="form-group"><label>Description / Instructions</label><textarea value={form.description} onChange={e=>upd('description',e.target.value)} placeholder="Clean all rooms, focus on bathrooms..." /></div>
