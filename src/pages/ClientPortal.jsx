...
export default function ClientPortal() {
  ...
  const loadAll = async () => {
    ...
    const [jobsRes, contractsRes, msgsRes, compRes, reqRes] = await Promise.all([
          ...
          <>
            ...
            {requests.length === 0 ? <div style={{ ...S.card, color: 'rgba(255,255,255,0.4)', fontSize: 13, textAlign: 'center' }}>{c.noRequests}</div> : requests.map(rq => (
              <div key={rq.id} style={S.card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ fontSize: 12, fontWeight: 700 }}>{rq.location_name || c.allLocations}</span>
                  <span style={{ fontSize: 11, color: rq.status === 'completed' ? '#4ade80' : '#fbbf24' }}>{rq.status === 'completed' ? c.statusDone : c.statusPending}</span>
                </div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginBottom: 6 }}>{new Date(rq.created_at).toLocaleDateString('ja-JP')}{rq.preferred_date ? ` · 希望: ${rq.preferred_date}` : ''}</div>
