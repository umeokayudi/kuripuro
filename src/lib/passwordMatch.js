...
export function hashPassword(value) {
  ...
  if (!pw) return ''
  return HASH_PREFIX + sha256Hex(pw)
}

export function isHashedPassword(stored) {
  return String(stored || '').startsWith(HASH_PREFIX)
}

/** Accepts legacy plaintext or sha256: hashes. Never rewrites the stored value. */
export function passwordMatches(stored, given) {
  const s = String(stored || '')
  const g = String(given || '')
  if (!g || !s) return false
  if (isHashedPassword(s)) return s === hashPassword(g)
  return s === g
}
